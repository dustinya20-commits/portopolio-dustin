...
import { ArrowDown, ArrowUpRight, Check, ChevronRight, Download, Github, Instagram, Layers3, LayoutTemplate, Linkedin, Menu, MousePointer2, PenTool, Quote, Search, TestTube2, X } from 'lucide-react';
import { portfolioData as data, type Project } from './data';

const iconMap = { search: Search, layout: LayoutTemplate, prototype: MousePointer2, system: Layers3, testing: TestTube2, visual: PenTool };
...
function App() {
  ...
  useEffect(() => {
    ...
    const description = 'Portfolio Salsa Pramudita, Junior UI/UX Designer dari Bandung. Eksplorasi riset, interface, dan pengalaman digital yang lebih manusiawi.';
    ...
    <div className="portfolio-shell grain bg-[#f5f5f5] text-[#0a0a0a]">
      ...
      <main id="main-content">
          ...
          <div className="mx-auto max-w-[1360px]">
            ...
            <div className="grid items-end gap-12 lg:grid-cols-[1fr_0.42fr]">
              <div className="pt-16 md:pt-24">
                <Reveal delay={0.08}><p className="mb-5 max-w-md text-sm leading-7 text-[#8a8a8a]">Hai, saya Salsa.<br />Saya mengubah pertanyaan menjadi pengalaman yang terasa tepat.</p></Reveal>
                <Reveal delay={0.14}><h1 id="hero-heading" className="display-font max-w-5xl text-[16vw] font-extrabold leading-[.82] tracking-[-0.09em] md:text-[11.3vw] lg:text-[10.5rem]">Salsa<br /><span className="text-[#8a8a8a]">Pramudita</span></h1></Reveal>
          ...
          <div className="mx-auto max-w-[1360px]">
            ...
            <div className="mt-12 grid gap-14 lg:grid-cols-[.72fr_1fr] lg:gap-24">
              ...
              <div className="grid gap-12 sm:grid-cols-[.8fr_1.2fr]">
                <Reveal delay={0.1}><PlaceholderArt className="aspect-[.8] w-full rounded-2xl" label="Placeholder potret grayscale untuk bagian tentang Salsa" /></Reveal>
                <Reveal delay={0.15}><div><p className="text-base font-semibold leading-7 md:text-lg">{data.person.intro}</p><p className="mt-6 text-sm leading-7 text-[#8a8a8a]">{data.person.about}</p><div className="mt-10 grid grid-cols-2 gap-y-8 border-t border-[#0a0a0a]/15 pt-8">{data.stats.map((stat) => <div key={stat.label}><div className="display-font text-3xl font-bold tracking-[-0.06em]">{stat.value}</div><div className="mt-1 text-[10px] uppercase tracking-[.12em] text-[#8a8a8a]">{stat.label}</div></div>)}</div></div></Reveal>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="keahlian" aria-labelledby="skills-heading" className="bg-white px-5 py-28 md:px-10 md:py-36">
          <div className="mx-auto max-w-[1360px]">
            <Reveal><div className="flex flex-wrap items-end justify-between gap-8"><div><SectionLabel>MY © SKILLS</SectionLabel><h2 id="skills-heading" className="display-font mt-8 text-4xl font-bold tracking-[-0.06em] md:text-6xl">Cara saya<br /><span className="text-[#8a8a8a]">bekerja.</span></h2></div><p className="max-w-xs text-sm leading-7 text-[#8a8a8a]">Bukan sekadar daftar kemampuan. Ini adalah cara saya mendekati masalah, satu lapis demi satu.</p></div></Reveal>
            <div className="mt-16 grid gap-3 md:grid-cols-2 lg:grid-cols-3">{data.skills.map((skill, index) => { const Icon = iconMap[skill.icon]; return <Reveal key={skill.title} delay={index * 0.05}><article className={`group min-h-56 rounded-2xl p-7 transition-transform duration-300 hover:-translate-y-1 ${index === 0 ? 'bg-[#0a0a0a] text-white' : 'bg-[#f5f5f5]'}`}><div className="flex items-start justify-between"><Icon size={27} strokeWidth={1.35} /><span className={`text-[10px] ${index === 0 ? 'text-[#8a8a8a]' : 'text-[#8a8a8a]'}`}>0{index + 1}</span></div><h3 className="mt-20 text-lg font-bold">{skill.title}</h3><p className={`mt-2 max-w-xs text-xs leading-6 ${index === 0 ? 'text-[#8a8a8a]' : 'text-[#8a8a8a]'}`}>{skill.description}</p></article></Reveal>; })}</div>
        ...
        <section id="proyek" aria-labelledby="work-heading" className="px-5 py-28 md:px-10 md:py-40">
          <div className="mx-auto max-w-[1360px]">
            <Reveal><div className="flex items-end justify-between gap-6"><div><SectionLabel>SELECTED © WORK</SectionLabel><h2 id="work-heading" className="display-font mt-8 text-4xl font-bold tracking-[-0.06em] md:text-6xl">Dipilih dengan<br /><span className="text-[#8a8a8a]">sengaja.</span></h2></div><span className="hidden text-[10px] uppercase tracking-[.13em] text-[#8a8a8a] md:block">04 / projects</span></div></Reveal>
